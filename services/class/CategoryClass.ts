...
export class CategoryService {
    ...
    static async upsert(props: UpsertCategoryProps): Promise<UpsertCategoryResponse> {
        try {
    ...
    static async count(props: CountCategoryProps): Promise<CountCategoryResponse> {
        try {
            const { cursor, orderBy, select, skip = 0, take = 10, where } = countCategorySchema.parse(props);
