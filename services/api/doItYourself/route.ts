...
const getDoItYourselfListCached = cache(
    async (stringParams: string): Promise<FindManyDoItYourselfResponse> => {
 ...
 */
export const GET = async (request: NextRequest): Promise<NextResponse<FindManyDoItYourselfResponse>> => {
    try {
        ...
        const response = await getDoItYourselfListCached(stringParams);
        ...
        if (process.env.NODE_ENV === "development") {
            if (error instanceof ZodError)
                return NextResponse.json({
                    error: "getDoItYourselfListCached -> Invalid Zod params -> " + error.message,
                });
